...
<div class="dashboard-container">
    <div class="top-bar">
        <div>
            <h1 class="page-title">Mentees Management</h1>
            <p style="color: var(--text-secondary); margin-top: 5px;">Manage registered mentees</p>
        </div>
                    ...
                    <tr>
                        <td>
                            <div style="display: flex; align-items: center; gap: 12px;">
                                <div style="width: 40px; height: 40px; border-radius: 12px; background: rgba(66, 153, 225, 0.1); color: #4299e1; display: flex; align-items: center; justify-content: center; font-weight: 700;">
                                    U
                                </div>
                                <div style="font-weight: 600; color: var(--text-primary);">{{ $mentee->name }}</div>
                            </div>
                        </td>
                        <td class="mobile-hide" style="color: var(--text-secondary);">{{ $mentee->email }}</td>
                        <td>
                            @if($mentee->email_verified_at)
                        ...
                        <td class="mobile-hide" style="color: var(--text-secondary);">
                            {{ $mentee->created_at->format('M j, Y') }}
                        </td>
                        <td>
                             <div style="display: flex; gap: 8px;">
                                <button onclick="openViewModal({{ $mentee }})" class="btn-icon" style="color: #667eea; background: rgba(102, 126, 234, 0.1); width: 32px; height: 32px; border-radius: 8px; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center;">
                                    👁️
                                </button>
                                <button onclick="openEditModal({{ $mentee }})" class="btn-icon" style="color: #ed8936; background: rgba(237, 137, 54, 0.1); width: 32px; height: 32px; border-radius: 8px; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center;">
                                    ✏️
                                </button>
                                <form action="{{ route('admin.users.delete', $mentee->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirm('Remove this mentee?')"
...
<div id="createModal" class="modal-backdrop">
    <div class="modal-content">
        <h3 style="font-size: 20px; font-weight: 700; margin-bottom: 20px;">Create New Mentee</h3>
        <form action="{{ route('admin.users.store') }}" method="POST">
            ...
            <div style="display: flex; justify-content: flex-end; gap: 10px;">
                ...
                <button type="submit" class="btn btn-primary">Create Mentee</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="modal-backdrop">
    <div class="modal-content">
        <h3 style="font-size: 20px; font-weight: 700; margin-bottom: 20px;">Edit Mentee</h3>
        <form id="editForm" method="POST">
            ...
            <div style="display: flex; justify-content: flex-end; gap: 10px;">
                ...
                <button type="submit" class="btn btn-primary">Update Mentee</button>
            </div>
        </form>
    </div>
</div>
...
<div id="viewModal" class="modal-backdrop">
    <div class="modal-content">
        <h3 style="font-size: 20px; font-weight: 700; margin-bottom: 20px;">Mentee Details</h3>
         <div style="display: grid; gap: 15px;">
            ...
            <div>
             ...
             <div>
                ...
                <span class="badge" style="background: rgba(66, 153, 225, 0.1); color: #4299e1;">Mentee</span>
            </div>
        </div>
